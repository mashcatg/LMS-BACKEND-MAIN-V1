...
function calculateDueAmount($conn, $enrollment_id, $service_id, $course_id) {
    try {
        ...
        foreach ($courses as $course) {
            ...
            $selectEnrollments = $conn->prepare("SELECT * FROM enrollments WHERE enrollment_id = :enrollment_id AND course_id = :course_id AND service_id = :service_id LIMIT 1");
            ...
            $total_payment = $conn->prepare("SELECT SUM(paid_amount) AS total_payment, SUM(discounted_amount) AS total_discount FROM payments WHERE enrollment_id = :enrollment_id AND service_id = :service_id");
            $total_payment->execute([':enrollment_id' => $enrollment_id, ':service_id' => $service_id]);
